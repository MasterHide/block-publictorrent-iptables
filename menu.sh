...
install_script() {
    ...
    if [ -f "/root/bt.sh" ]; then
        echo -e "${YELLOW}script is already installed.${RESET}"
    else
...
uninstall_script() {
    ...
    if [ -f "/root/bt.sh" ]; then
        echo -e "${RED}Removing Script...${RESET}"
        rm -f /root/bt.sh
    else
        echo -e "${YELLOW}path not found in /root.${RESET}"
    fi
    ...
    echo -e "${GREEN}Uninstallation complete baby.${RESET}"
}
...
menu() {
    ...
    echo -e "${BLUE}----------------------------------${RESET}"
    echo -e "${GREEN}1.Install Script${RESET}"
    echo -e "${RED}2. Uninstall Script${RESET}"
